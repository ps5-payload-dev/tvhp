...
// Converts an SDL game controller button (SDL_GameControllerButton) to the
// RmlUi key it should act as, so controller input drives the exact same
// keydown/keyup path as the keyboard (d-pad = arrow keys, south button =
// Return, east button = Escape). Returns KI_UNKNOWN for unmapped buttons.
Rml::Input::KeyIdentifier ConvertControllerButton(int sdl_controller_button);
